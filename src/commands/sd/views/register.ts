  ...
  execute: async (
    ...
    try {
      ...
      const exists = await pg
        ...
        .where(
          and(
            eq(yswsUsers.accId, flatValues["acc_name"]!),
      ...
      if (view.state.values?.["jobs"]?.["jobs"]?.selected_options) {
        for (const job of view.state.values?.["jobs"]?.["jobs"]
          ?.selected_options) {
          if (
            ysws.stardance.jobConfig[job.value] &&
            ysws.stardance.jobConfig[job.value]?.apiKeyRequired &&
          ...
          if (
            ysws.stardance.jobConfig[job.value] &&
            ysws.stardance.jobConfig[job.value]?.channelRequired &&
      ...
      await client.chat.postEphemeral({
        ...
        user: userId,
        markdown_text:
          "You are now able to use all Stardance based commands! :yay:",
      });
