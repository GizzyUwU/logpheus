import { pgTable, text, integer, boolean, real, primaryKey, index } from "drizzle-orm/pg-core";
...
export const yswsUsers = pgTable("ysws", {
  ...
  meta: text().array(),
  registeredJobs: text().array()
}, (table) => [
  primaryKey({ columns: [table.yswsId, table.userId] }),
  index("users_registered_jobs_idx")
    .using("gin", table.registeredJobs),
]);
